...
class PAML_Class {
    ...
    public function __construct() {
        ...
        // Custom styles
        add_action('wp_head', array($this, 'paml_print_style_css'));

        // Add the users custom CSS if they wish to add any.
        if ($paml_options['custom-css'])
            add_action('wp_head', array($this, 'paml_print_custom_css'));
        ...
        // Register widget.
        add_action('widgets_init', create_function('', 'register_widget( "PAML_Widget" );'));
    ...
    public function paml_add_nav_menu_metabox() {
        add_meta_box('paml_metabox_modal_link', __('PA Modal Link'), array($this, 'paml_callback_metabox_modal_link'), 'nav-menus', 'side', 'high');
        ...
        $strings = array(
            'label_login' => __('Login Label', 'pressapps'),
            'label_logout' => __('Logout Label', 'pressapps'),
        );
    ...
    public function paml_resources() {
        ...
        wp_enqueue_style('paml-styles', PAML_PLUGIN_ASSETS_URL . 'css/modal-login.css', null, $this->plugin_version, 'screen');

        // Load the right Modal Theme
        if (!isset($theme) || $theme == 'default') {
            
        } elseif ($theme == 'wide') {
            wp_enqueue_style('theme-wide', PAML_PLUGIN_ASSETS_URL . 'css/theme-wide.css', null, $this->plugin_version, 'screen');
        }
        ...
        if (!is_user_logged_in()) {
            wp_localize_script('paml-script', 'modal_login_script', array(
                ...
                'redirecturl' => apply_filters('modal_login_redirect_to', $login_url),
                'loadingmessage' => __('Checking Credentials...', 'pressapps'),
            ));
        }
    }

    public function paml_print_style_css() {
        global $paml_options;

        echo '<style text="text/css" id="modal-login-style-css">';
        if ($paml_options['bkg-color']) {
            echo '.ml-content { background-color: ' . sanitize_text_field($paml_options['bkg-color']) . "}\n";
            if ($paml_options['bkg-color'] != '#ffffff') {
                echo '#modal-login input[type="text"], #modal-login input[type="password"] { border: solid 1px ' . $paml_options['bkg-color'] . "; }\n";
                echo '#modal-login input[type="text"]:focus, #modal-login input[type="password"]:focus { border: solid 1px ' . $paml_options['bkg-color'] . "; }\n";
            }
        }
        if ($paml_options['font-color']) {
            echo '#modal-login, #modal-login h2 { color: ' . sanitize_text_field($paml_options['font-color']) . "}\n";
        }
        if ($paml_options['link-color']) {
            echo '#additional-settings, #additional-settings a, #additional-settings a:hover { color: ' . sanitize_text_field($paml_options['link-color']) . "}\n";
        }
        if ($paml_options['btn-color']) {
            echo '#modal-login .submit .button { background: ' . sanitize_text_field($paml_options['btn-color']) . "}\n";
    ...
    public function paml_print_custom_css() {
        global $paml_options;

        echo '<style text="text/css" id="modal-login-custom-css">' . sanitize_text_field($paml_options['custom-css']) . "</style>\n";
    }
    ...
    function paml_register_new_user($user_login, $user_email) {
        global $paml_options;
        $labels = $paml_options['modal-labels'];
        ...
        <div id="modal-login" class="ml-modal fade" tabindex="-1" role="dialog" aria-hidden="true">
                    ...
                    <div class="ml-content">
                        ...
                        <div class="section-container">
                            ...
                            <div id="login" class="modal-login-content">
                                <div class="fix-btn-connect">
                                    <a href="http://localhost:8888/hanoivoyage/wp-login.php?loginFacebook=1&redirect=http://localhost:8888/hanoivoyage" onclick="window.location = 'http://localhost:8888/hanoivoyage/wp-login.php?loginFacebook=1&redirect=' + window.location.href;
                                                    return false;"> <div class="new-fb-btn new-fb-3 new-fb-default-anim"><div class="new-fb-3-1"><div class="new-fb-3-1-1">CONNECT</div></div></div> </a>
                                    <a href="http://localhost:8888/hanoivoyage/wp-login.php?loginGoogle=1&redirect=http://localhost:8888/hanoivoyage" onclick="window.location = 'http://localhost:8888/hanoivoyage/wp-login.php?loginGoogle=1&redirect=' + window.location.href;
                                                    return false;"><div class="new-google-btn new-google-3 new-google-default-anim"><div class="new-google-3-1"><div class="new-google-3-1-1">CONNECT</div></div></div></a>
                                </div>
                                <button type="button" class="ml-close" data-dismiss="ml-modal" aria-hidden="true">&times;</button>
                                <div style="text-align: center"><h4><?php _e('Or', 'pressapps'); ?></h4></div>
                                ...
                                <form action="login" method="post" id="form" class="group" name="loginform">
                                    ...
                                    <p class="mluser">
                                        ...
                                        <input type="text" name="log" id="login_user" class="input" placeholder="<?php
                                        if ($labels == 'placeholders') {
                                            _e('Username', 'pressapps');
                                        }
                                        ?>" value="<?php if (isset($user_login)) echo esc_attr($user_login); ?>" size="20" />
                                    </p>

                                    <p class="mlpsw">
                                        <label class="field-titles" for="login_pass"><?php _e('Password', 'pressapps'); ?></label>
                                        <input type="password" name="pwd" id="login_pass" class="input" placeholder="<?php
                            if ($labels == 'placeholders') {
                                _e('Password', 'pressapps');
                            }
                                        ?>" value="" size="20" />
                                    </p>
                                    ...
                                    <div id="forgetmenot">
                                        <label class="forgetmenot-label" for="rememberme"><input name="rememberme" type="checkbox" placeholder="<?php
                                if ($labels == 'placeholders') {
                                    _e('Password', 'pressapps');
                                }
                                            ?>" id="rememberme" value="forever" /> <?php _e('Remember Me', 'pressapps'); ?></label>
                                    </div>
                ...
                <?php do_action('before_modal_register'); ?>
                                    ...
                                    <form action="register" method="post" id="form" class="group" name="loginform">
                                        ...
                                        <p class="mluser">
                                            ...
                                            <input type="text" name="user_login" id="reg_user" class="input" placeholder="<?php
                                   if ($labels == 'placeholders') {
                                       _e('Username', 'pressapps');
                                   }
                                                   ?>" value="<?php if (isset($user_login)) echo esc_attr(stripslashes($user_login)); ?>" size="20" />
                                        </p>

                                        <p class="mlemail">
                                            <label class="field-titles" for="reg_email"><?php _e('Email', 'pressapps'); ?></label>
                                            <input type="text" name="user_email" id="reg_email" class="input" placeholder="<?php
                                   if ($labels == 'placeholders') {
                                       _e('Email', 'pressapps');
                                   }
                                                   ?>" value="<?php if (isset($user_email)) echo esc_attr(stripslashes($user_email)); ?>" size="20" />
                                        </p>
                <?php
                if (isset($paml_options['userdefine_password'])) {
                    ?>
                                            <p class="mlregpsw">
                                                ...
                                                <input type="password" name="reg_password" id="reg_password" class="input" placeholder="<?php
                                            if ($labels == 'placeholders') {
                                                _e('Password', 'pressapps');
                                            }
                                            ?>"  />
                                            ...
                                            <p class="mlregpswconf">
                                                ...
                                                <input type="password" name="reg_cpassword" id="reg_cpassword" class="input" placeholder="<?php
                                            if ($labels == 'placeholders') {
                                                _e('Confirm Password', 'pressapps');
                                            }
                                            ?>"  />
                                            </p>
                    <?php
            ...
            <?php do_action('inside_modal_forgotton_first'); ?>
                                    ...
                                    <p class="mlforgt">
                                        ...
                                        <input type="text" name="forgot_login" id="forgot_login" class="input" placeholder="<?php
                            if ($labels == 'placeholders') {
                                _e('Username or Email', 'pressapps');
                            }
            ?>" value="<?php if (isset($user_login)) echo esc_attr(stripslashes($user_login)); ?>" size="20" />
                                    </p>
    ...
    public function paml_additional_options() {
        ...
        echo '<div id="additional-settings" class="mini-content">';
        ...
        if (( get_option('users_can_register') && !is_multisite() ) || ( $multisite_reg == 'all' || $multisite_reg == 'blog' || $multisite_reg == 'user' ))
            echo '<div class="reg-skin-custom"><div class="border-distance"></div><div class="tran-lag-reg">';

            echo do_shortcode('[cml_text en="Not a member?" fr="Pas encore membre?"]');

            echo '</div><div class="text-reg"><a href="#register" class="modal-login-nav">' . __('Register', 'pressapps') . '</a> </div></div>';
    }
                ...
                <ul id ="paml-modal-link-checklist" class="categorychecklist form-no-clear">
                    <li>
                        <label class="menu-item-title">
                            <input type="checkbox" class="menu-item-checkbox" name="menu-item[-1][menu-item-object-id]" value="-1"> <?php _e('Login', $paml_textdomain); ?> / <?php _e('Logout', $paml_textdomain); ?>
                        </label>
                        <input type="hidden" class="menu-item-type" name="menu-item[-1][menu-item-type]" value="custom">
                        <input type="hidden" class="menu-item-title" name="menu-item[-1][menu-item-title]" value="<?php _e('Login', $paml_textdomain); ?> // <?php _e('Logout', $paml_textdomain); ?>">
                        <input type="hidden" class="menu-item-url" name="menu-item[-1][menu-item-url]" value="#pa_modal_login">
                    </li>
                    <li>
                        <label class="menu-item-title">
                            <input type="checkbox" class="menu-item-checkbox" name="menu-item[-1][menu-item-object-id]" value="-1"> <?php _e('Register', $paml_textdomain); ?>
                        </label>
                        <input type="hidden" class="menu-item-type" name="menu-item[-1][menu-item-type]" value="custom">
                        <input type="hidden" class="menu-item-title" name="menu-item[-1][menu-item-title]" value="<?php _e('Register', $paml_textdomain); ?>">

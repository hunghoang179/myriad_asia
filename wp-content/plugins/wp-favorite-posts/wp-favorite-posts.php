...
function wpfp_link_html($post_id, $opt, $action) {   
    ...
    if ($action == "remove"):
        ...
        $link = "<a id='rem_$post_id' class='$class' href='?wpfpaction=remove&amp;page=1&amp;postid=" . $post_id . "' title='" . wpfp_get_option('rem') . "' rel='nofollow'>" . $str . "</a>";
        $link = apply_filters('wpfp_remove_favorite_link', $link);
        return $link;
    else:
        $str .= wpfp_before_link_img();
        $link = "<a class='wpfp-link' href='?wpfpaction=" . $action . "&amp;postid=" . $post_id . "' title='" . $opt . "' rel='nofollow'>" . $str . "</a>";
...
function wpfp_clear_list_link() {
    if (wpfp_is_user_can_edit()) {
        ...
        $str .= wpfp_loading_img();
        $str .= wpfp_after_img();
        echo "<a class='wpfp-link' href='?wpfpaction=clear' rel='nofollow'>" . wpfp_get_option('clear') . $str . "</a>";
    }
}

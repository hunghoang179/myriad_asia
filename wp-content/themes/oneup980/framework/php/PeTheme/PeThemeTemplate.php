...
class PeThemeTemplate {
    ...
    public function comment_form() {
        $loop = false;
        if ($this->custom($loop, "comment-form"))
            return;
        ?>
        <?php $t = & peTheme(); ?>
                    ...
                    <div class="row-fluid">
                        <div class="row">
                            <form action="<?php $comments->action(); ?>" method="post" id="commentform" class="form-horizontal">
                                ...
                                <?php else: ?>
                                    <p class="comment-notes col-md-12"><?php echo __('Your email address will not be published. Required fields are marked', 'Pixelentity Theme/Plugin'); ?> <span class="required">*</span></p>
                                    <div class="comment-form-author control-group col-md-3">
                                        <label for="author"><?php _e("Name", 'Pixelentity Theme/Plugin'); ?> <span class="required">*</span></label>

                                        <input class="span12" id="author" name="author" type="text" value="" size="30" aria-required="true"/>
                                        <small class="error"></small>


                                    </div>

                                    <div class="comment-form-email control-group col-md-3">
                                        <label for="email"><?php _e("Email", 'Pixelentity Theme/Plugin'); ?> <span class="required">*</span></label>

                                        <input class="span12" id="email" name="email" type="text" value="" size="30" aria-required="true"/>
                                        <small class="error"></small>


                                    </div>

                                    <div class="comment-form-url control-group col-md-3">
                                        <label for="url"><?php _e("Website", 'Pixelentity Theme/Plugin'); ?></label>

                                        <input class="span12" id="url" name="url" type="text" value="" size="30"/>
                                        <small class="error"></small>
                                ...
                                <div class="comment-form-comment control-group">
                                    <div class="controls col-md-12">
                                        ...
                                        <textarea class="span9" id="comment" name="comment" cols="45" rows="8" aria-required="true" placeholder="<?php _e("Your comment here..", 'Pixelentity Theme/Plugin'); ?>"></textarea>
                                    </div>
                                </div>
    ...
    public function slider_volo($slider, $name = "gallery") {
        if (!$slider || !isset($slider->main->loop))
            return "";
    ...
    public function gallery_cover($w, $h, $galopts = null) {
        $loop = false;
        if ($this->custom($loop, "gallery-cover"))
            return;
        $t = & peTheme();
        ...
        $info = sprintf('<div class="title"><span>%s</span>%s</div>', apply_filters("pe_theme_gallery_cover_count", " &times; $count", $count), $title ? sprintf('<a href="%s">%s</a>', get_permalink(), $title) : '<i></i>'
        );
        ...
        <div class="portfolioItem galleryCover">
            <?php if ($fullscreen && !$link): ?>
                ...
                <?php else: ?>		
                    <a class="<?php echo $overClass; ?>" href="<?php
                    if ($link) {
                        echo $link;
                    } else {
                        $t->content->link();
    ...
    public function paginate_links($loop) {
        ...
        ?>
        <div class="<?php echo $classes; ?>" <?php echo $all ?> data-msg="<?php _e("Load More", 'Pixelentity Theme/Plugin'); ?>">
            <div class="<?php echo $loop->main->class ?>">
                <div class="pagination">
                    <ul>
                        <li class="<?php echo $loop->main->prev->class ?>">
                            <a href="<?php echo $loop->main->prev->link ?>"><?php _e("Prev", 'Pixelentity Theme/Plugin'); ?></a>
                        </li>
                        ...
                        <li class="<?php echo $loop->main->next->class ?>">
                            <a href="<?php echo $loop->main->next->link ?>"><?php _e("Next", 'Pixelentity Theme/Plugin'); ?></a>
                        </li>
                    </ul>
                </div>  
            </div>

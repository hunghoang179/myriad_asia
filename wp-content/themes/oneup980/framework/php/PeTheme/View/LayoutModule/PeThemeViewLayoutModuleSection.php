...
class PeThemeViewLayoutModuleSection extends PeThemeViewLayoutModuleContainer {
    ...
    public function render() {
        ...
        if ($data->bg === 'color' || $data->bg === 'imagecolor') {
            ...
            $color = $data->color;
            $rgba = sprintf(
                    "rgba(%s,%s,%s,%s)", hexdec(substr($color, 1, 2)), hexdec(substr($color, 3, 2)), hexdec(substr($color, 5, 2)), intval($data->alpha) / 100);
        ...
        echo sprintf('<section class="section pe-main-section pe-view-layout-block pe-view-layout-block-%s %s" id="section-%s"%s>', $this->conf->bid, $classes, $id, $style);
        $this->template();
        echo '</section>';
    }

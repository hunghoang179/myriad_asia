...
class PeThemeVManager {
    ...
    public function post_types() {
        if (!isset($this->_post_types)) {
            foreach (get_post_types('', 'objects') as $name => $pt) {
                if (in_array($name, array("revision", "nav_menu_item", "view", "gallery")))
                    continue;
    ...
    public function pe_theme_metabox_config_view() {
        ...
        foreach ($types as $name => $type) {
            ...
            PeGlobal::$config["metaboxes-view"]["view-$type"] = array(
        ...
        PeGlobal::$config["metaboxes-view"]["caption-gallery"] = array(
        ...
        PeGlobal::$config["metaboxes-view"]["caption-post"] = array(
                    "title" => __("Caption Settings", 'Pixelentity Theme/Plugin'),
                    "priority" => "core",
                    "where" =>
                    array(
    ...
    public function add_meta_boxes_view() {
        ...
        wp_localize_script('pe_theme_view', 'pe_theme_view', array(
            "captions" => $captions,
            "links" => $links,
            "taxonomies" => $taxmap
                )
    ...
    public function option($skipcurrent = true, $types = false, $group = true) {
        ...
        if (count($posts) > 0) {
            $options = array();
            foreach ($posts as $post) {
                if (intval($post->ID) === $currentID)
                    continue;
                //$meta = $this->master->meta->get($post->ID,$post->post_type);
                $meta = maybe_unserialize(get_post_meta($post->ID, PE_THEME_META, true));
                if (empty($meta) || empty($views[$meta->{"view-" . $meta->data->type}->type]))
                    continue;
    ...
    public function caption($caption) {
        ...
        if ($html = do_shortcode(apply_filters("the_content", $caption))) {
            printf(
                    '<div class="peCaption"%s>', $size ? sprintf(' data-orig-width="%s" data-orig-height="%s"', $size[0], $size[1]) : ""
            );
            echo $html;
            echo '</div>';
        }

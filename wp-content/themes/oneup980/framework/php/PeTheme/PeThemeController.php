...
class PeThemeController {
    ...
    public function boot() {
        ...
        PeGlobal::$config["content-width"] = 980;
        ...
        PeGlobal::$config["columns"] = array(
            __("1 Column layouts", 'Pixelentity Theme/Plugin') =>
            array(
                __("1/1", 'Pixelentity Theme/Plugin') => "1/1", /* span12 */
            ),
        ...
        $options = array(
            ...
            array(
                ...
                "description" => __('Enables  "Load More" feature in default blog archives, tags and category pages. Ignored in Views since they have a dedicated option to activate the feature.', 'Pixelentity Theme/Plugin'),
                "options" => Array(__("Yes", 'Pixelentity Theme/Plugin') => "yes", __("No", 'Pixelentity Theme/Plugin') => "no"),
                "default" => "no"
            ),
            "builderInContent" =>
            array(
                "label" => __("Save Builder in Editor", 'Pixelentity Theme/Plugin'),
                "type" => "RadioUI",
                "section" => __("Advanced", 'Pixelentity Theme/Plugin'),
                "description" => __('If enabled, the page builder content will replace editor content when saving the page. Plugins like Yoast SEO will be then able to access builder content as well.', 'Pixelentity Theme/Plugin'),
                "options" => Array(__("Yes", 'Pixelentity Theme/Plugin') => "yes", __("No", 'Pixelentity Theme/Plugin') => "no"),
                "default" => "no"
            ),
            "lazyImages" =>
    ...
    public function pe_theme_resized_img_filter($markup, $url, $w, $h) {
        ...
        return sprintf('<img class="peLazyLoading" src="%s" data-original="%s" width="%s" height="%s" alt="" />', $this->image->blank($w, $h), $url, $w, $h
        );
    }

    public function pe_theme_resized_retina_filter($markup, $url, $w, $h, $unscaled) {

        return sprintf('<img class="peLazyLoading" src="%s" data-original="%s" data-original-hires="%s" width="%s" height="%s" alt="" />', $this->image->blank($w, $h), $url, $this->image->resizedImgUrl($unscaled, $w * 2, $h * 2), $w, $h
        );
    }

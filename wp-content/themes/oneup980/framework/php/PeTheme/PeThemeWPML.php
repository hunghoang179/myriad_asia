...
class PeThemeWPML {
    ...
    public function dynamic_sidebar_params_filter($instance, $widget, $args) {
        ...
        if (!empty($this->sidebars[$sb]) && ICL_LANGUAGE_CODE != $this->sidebars[$sb])
            return false;
        return $instance;
    }
        ...
        if ($depth === 0 && $item->url === "#wpml") {
            ...
            if (!empty($langs) && is_array($langs)) {
                ...
                foreach ($langs as $lang) {
                    $output .= sprintf('<a class="pe-wpml-flag" href="%s"><img src="%s" alt="%s" /></a>', $lang['url'], $lang['country_flag_url'], $lang['native_name']
                    );
                }
                $item_output = $output;
            }
